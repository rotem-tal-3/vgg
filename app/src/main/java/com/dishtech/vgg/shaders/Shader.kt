...
class fibo(override val uniforms: Array<Uniform>) : SpectrumShader, SchemeShader, TimedShader {
	constructor(scheme: Bitmap, spectrum: FloatArray, iTime: Float, aspectRatio: Float): this(arrayOf(Uniform("scheme", "sampler2D", scheme), Uniform("spectrum", "float[]", spectrum), Uniform("iTime", "float", iTime), Uniform("aspectRatio", "float", aspectRatio)))
	override val name = "fibo"
	override var scheme get() = uniforms[0].value as Bitmap
		set(value) {
			uniforms[0].value = value
			ShaderManager.setValueForShader(name, uniforms[0].name, value)
	...
	override var iTime get() = uniforms[2].value as Float
		set(value) {
			uniforms[2].value = value
			ShaderManager.setValueForShader(name, uniforms[2].name, value)
		}
	var aspectRatio get() = uniforms[3].value
		set(value) {
